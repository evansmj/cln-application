...
describe('ConnectWallet component ', () => {
  ...
  it('updates selected network and input fields on network change to REST (Tor)', async () => {
    ...
    expect(screen.getByTestId('host')).toHaveValue(
      'http://oqaer4kd7ufryngx6dsztovs4pnlmaouwmtkofjsd2m7pkq8wd.onion',
    );
    expect(screen.getByTestId('macaroon')).toHaveValue(
      '0201036c6e6402e501030a1042beb666ba043f72cb147adf3eaafc9e1201301a160a076164647265737312047265616',
    );
    expect(screen.queryByTestId('client-cert')).not.toBeInTheDocument();
    expect(screen.queryByTestId('ca-cert')).not.toBeInTheDocument();
    expect(screen.getByTestId('connect-url')).toHaveValue(
      'c-lightning-rest://http://oqaer4kd7ufryngx6dsztovs4pnlmaouwmtkofjsd2m7pkq8wd.onion:3001?macaroon=0201036c6e6402e501030a1042beb666ba043f72cb147adf3eaafc9e1201301a160a076164647265737312047265616&protocol=http',
    );
    expect(screen.queryByTestId('invoice-rune')).not.toBeInTheDocument();
  });
  ...
  it('updates selected network and input fields on network change to gRPC', async () => {
    ...
    expect(screen.getByTestId('ca-cert')).toHaveValue('CACert');
    expect(screen.getByTestId('connect-url')).toHaveValue(
      'cln-grpc://user.local:2106?clientkey=ClientKey&clientCert=ClientCert&caCert=CACert',
    );
    expect(screen.queryByTestId('invoice-rune')).not.toBeInTheDocument();
  });
  ...
  it('updates selected network and input fields on network change to gRPC (Tor)', async () => {
    ...
    expect(screen.getByTestId('port')).toHaveValue('2106');
    expect(screen.getByTestId('host')).toHaveValue(
      'oqaer4kd7ufryngx6dsztovs4pnlmaouwmtkofjsd2m7pkq8wd.onion',
    );
    expect(screen.getByTestId('macaroon')).toHaveValue('ClientKey');
    expect(screen.getByTestId('client-cert')).toHaveValue('ClientCert');
    expect(screen.queryByTestId('ca-cert')).not.toBeInTheDocument();
    expect(screen.getByTestId('connect-url')).toHaveValue(
      'cln-grpc://oqaer4kd7ufryngx6dsztovs4pnlmaouwmtkofjsd2m7pkq8wd.onion:2106?clientkey=ClientKey&clientCert=ClientCert',
    );
    expect(screen.queryByTestId('invoice-rune')).not.toBeInTheDocument();
  });
